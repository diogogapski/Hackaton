...
export function NextChallengeSection({ dados }: { dados: DadosHome }) {
  const desafios = dados?.desafios ?? [];
  const destaque = desafios[0] ?? null;
  const outros = desafios.length - 1;
  ...
  return (
    <section id="desafios" className="relative overflow-hidden border-t border-foreground/10 bg-background py-24 lg:py-32">
      ...
      <div className="relative mx-auto max-w-[1440px] px-6 md:px-10">
        <p className="font-display text-[0.76rem] font-semibold uppercase tracking-[0.08em] text-foreground/55"><span className="text-accent">03 //</span> {destaque ? "DESAFIOS DA EDIÇÃO" : "PRÓXIMO DESAFIO"}</p>
        <div className="mt-7 grid gap-8 lg:grid-cols-[0.58fr_0.42fr] lg:items-end">
          <h2 className="font-display text-[clamp(2.25rem,5vw,5.4rem)] font-semibold uppercase leading-[0.98] tracking-0">
            {destaque ? (
              <>
                <span className="block text-foreground">O PROBLEMA</span>
                <span className="block text-accent">JÁ ESTÁ NA MESA._</span>
              </>
            ) : (
              <>
          ...
          <p className="max-w-[520px] font-display text-base leading-7 text-foreground/64 md:text-[1.05rem] lg:justify-self-end">
            {destaque
              ? `${desafios.length} desafio(s) publicado(s) para ${dados?.nome}. Escolha o seu e monte a equipe.`
              : "Novos desafios serão publicados aqui. Prepare sua equipe. A próxima ideia pode começar com vocês."}
          </p>
        </div>
        <div className="relative mt-16 min-h-[420px] border border-foreground/12 bg-background/92 p-6 md:p-10 lg:p-12">
          ...
          <div className="flex h-full min-h-[340px] flex-col">
            <div className="flex items-start justify-between gap-6">
              ...
              <p className="font-display text-[0.72rem] uppercase text-foreground/35">DATA_SOURCE // {destaque ? "HACKIF.API" : "LOCAL"}</p>
            </div>
            <div className="mt-12">
              <p className="font-display text-[0.72rem] uppercase tracking-[0.08em] text-foreground/45">STATUS</p>
              <div className="mt-3 flex items-center gap-3 font-display text-[1rem] font-semibold uppercase text-accent"><span className="h-2.5 w-2.5 rounded-full bg-accent" aria-hidden="true" />{destaque ? "PUBLICADO" : "EM BREVE"}</div>
            </div>
            {destaque ? (
              <div className="flex-1 py-10">
                {destaque.categoria ? <p className="font-display text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-foreground/45">{destaque.categoria}</p> : null}
                <h3 className="mt-3 font-display text-[clamp(2rem,4.5vw,4.2rem)] font-semibold uppercase leading-[1] text-foreground">{destaque.titulo}</h3>
                <p className="mt-6 line-clamp-4 max-w-[760px] whitespace-pre-line font-display text-[1rem] leading-7 text-foreground/62">{destaque.descricao}</p>
              </div>
            ) : (
            ...
            <div className="mt-auto flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
              <p className="max-w-[460px] font-display text-[1rem] leading-7 text-foreground/62">
                {destaque ? (outros > 0 ? `+ ${outros} outro(s) desafio(s) nesta edição.` : "Desafio único desta edição.") : "O próximo desafio está sendo preparado."}
              </p>
              <a href="/hackathon#desafios" className="font-display text-[0.82rem] font-semibold uppercase text-accent transition-colors hover:text-foreground">VER DESAFIOS ↗</a>
            </div>
          </div>
        </div>
      </div>
